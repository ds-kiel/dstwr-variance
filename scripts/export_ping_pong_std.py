...
def export_first_std_graph(export_dir):
    ...
    if alt_pred is not None:
        ax2.plot(data_xs_ratio, alt_pred, alpha=1.0, linestyle='dotted', color='gray', label='DS-TWR Model\n[Navrátil & Vejražka]')
    ...
    line = Line2D([0], [0], label='Proposed Model', color='gray', linestyle='--')
...
def export_second_std_graph(export_dir):
    assume_twr_equal_noise = False
    ...
    save_and_crop("{}/std_fit_ping_pong_second_graph.pdf".format(export_dir), bbox_inches='tight', crop=True)  # , pad_inches=0)
    plt.close()


def export_third_std_graph(export_dir):
    assume_twr_equal_noise = False
    ...
    if alt_pred is not None:
        ax.plot(data_xs_ratio, alt_pred, alpha=1.0, linestyle='dotted', color='gray', label='DS-TWR Model\n[Navrátil & Vejražka]')

    ax.plot(data_xs_ratio, pred_twr_ys, alpha=1.0, linestyle='--', color=colors[responder])


    # label='Fit ToF $(\sigma_{{AB}}={:.2f}, \sigma_{{BA}}={:.2f})$'.format(popt[0]*100, popt[1]*100))

    active_df_aggr.plot.line(y='twr_tof_ds_err', ax=ax, label="DS-TWR ${}\\rightarrow {}$".format(initiator+1, responder+1), style='-', color=colors[responder])
    ...
    for (i, passive_dev) in enumerate(passive_devs):
        ...
        pax.plot(data_xs_ratio, pred_tdoa_ys, color=colors[passive_dev], linestyle='--', alpha=1.0)
        # label='Fit TDoA $(\sigma_{{AL}}={:.2f}, \sigma_{{BL}}={:.2f})$'.format(passive_popt[0]*100, passive_popt[1]*100),
        aggr_filt_df.plot.line(y='tdoa_est_ds', ax= ax, label="DS-TDoA ${}$".format(passive_dev + 1), color=colors[passive_dev], style=styles[passive_dev], marker=markers[passive_dev], markersize=marker_size)

    ax.yaxis.set_major_formatter(lambda x, pos: np.round(x * 100.0, 1))  # scale to cm
    fig.set_size_inches(5.0, 6.0+0.1)
    ...
    ax.set_ylim(0.15, 0.25) # outliers
    #ax2.set_ylim(None, 0.05)
    ...
    save_and_crop("{}/std_fit_ping_pong_third_graph.pdf".format(export_dir), bbox_inches='tight', crop=True)  # , pad_inches=0)
    plt.close()
